(function($) {
  ...
  function setVis() {
    ...
    columns = ['Timeline', 'Smallpox','Rubella','Mumps','Hepatitis A', 'Pertussis','Flu', 'Measles', 'Chickenpox', 'Polio'];
  ...
  function resize() {
    ...
    if (width < 415) {
      x.domain(['Timeline', 'Smallpox','Rubella','Mumps','Hepatitis A','Flu', 'Measles', 'Chickenpox', 'Polio']);
    } else if (width < 500) {
      margin = {
        top: 15,
        left: 40
    ...
    hsvg.selectAll('.scale')
      .attr('transform', function(d, i) {
        ...
        return (width < 500) ? "translate(" + (size(5000) + 5) + ',' + (40 + size(5000) * 2 - size(d) + 20) + ")" : "translate(" + (size(5000) + 5) + ',' + (75 + size(5000) * 2 - size(d) + 20) + ")"
      });
    ...
    svg.selectAll('.yr-txt')
      .attr('transform', function(d) {
        return (width < 500) ? "translate(" + 30 + "," + y(d.year) + ")" : "translate(" + (x('Timeline') - 10) + "," + y(d.year) + ")";
      })
  ...
  function setScales(data) {
    ...
    tip = d3.tip()
      ...
      .html(function(d) {
        return "<p>Month: <strong>" + d.years + "</strong></p> <p>Disease: <strong>" + d.disease + "</strong></p>" +
  ...
  function drawViz(data) {
    ...
    var scale = hsvg.selectAll('.scale')
      ...
      .attr('class', 'scale')
      .attr('transform', function(d, i) {
        return (width < 500) ? "translate(" + (size(5000) + 5) + ',' + (40 + size(5000) * 2 - size(d) + 20) + ")" : "translate(" + (size(5000) + 2) + ',' + (75 + size(5000) * 2 - size(d) + 20) + ")"
      });
    ...
    scale.append("text")
      ...
      .attr('x', (size(5000) + 10))
      .text(function(d) {
        // return commaFormat(d/100);
        return d3.format(".0%")(d/10000);
  ...
  // .style("fill", function(d) { return color(cValue(d));})
    ...
    var coltxt = svg.selectAll('.col-txt')
      ...
      .attr('transform', function(d) {
      return "translate(" + x(d.disease) + "," + (y(d.year) -50) + ")";
      })
      .append('text')
      .text(function(d) {
        return d.disease
      ...
      var coltxt2 = svg.selectAll('.col-txt2')
        ...
        .style("font-size","10px")
        .attr('transform', function(d) {
        return "translate(" + x(d.disease) + "," + (y(d.year) -30) + ")";
        })
        .append('text')
        .text(function(d) {
          return d.date
        ...
        var coltxt3 = svg.selectAll('.col-txt3')
          .data(data.filter(function(d) {
            return d.head == "TRUE"
          }))
          // .data(data)
